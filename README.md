...
- **Boot-Drive Safety**: disk detection produces three lists:
  - `_protected_disks` (strict) — disks hosting an OS / system mount (`/`, `/boot`, `/boot/efi`, `/usr`, `/var`, `/home`, swap, etc.) via an explicit allow-list. EXCLUDED from discovery. Empty list hard-fails the playbook unless `allow_empty_protected_disks: true`. **Data mounts (e.g. `/hammerspace/*`) are intentionally NOT walked** — otherwise re-runs on hosts with an existing `/dev/md127` mounted at `/hammerspace/hsvol0` would mis-classify `sdb`/`sdc` as protected via the md→members inverse-lsblk walk.
  ...
  A final assertion before any `mkfs` rejects candidates that overlap `_protected_disks`. Regression tests: `tests/integration/test_boot_device_safety.yml`, `tests/integration/test_protected_vs_md_split.yml` (5 cases).
...
> **Recommended invocation: `./scripts/run.sh playbook ...`.** The wrapper:
> - Auto-detects an installed UTF-8 locale (`en_US.UTF-8` → `C.UTF-8` → `en_US.utf8` → `C.utf8`) — required on non-English hosts (Korean / CJK / etc.) where Ansible would otherwise abort with `ERROR: Ansible could not initialize the preferred locale`.
> - Defaults `ANSIBLE_LOG_PATH=${HOME}/logs/ansible-<timestamp>.log` so every run is logged (shell aliases on `ansible-playbook` don't propagate through `exec`, so this is the only reliable way).
> - Prints the active locale + log path to stderr on every invocation.
>
> Overrides:
> ```bash
...
> ```
> Or skip the wrapper and configure manually:
> ```bash
> export LANG=C.UTF-8 LC_ALL=C.UTF-8
> export ANSIBLE_LOG_PATH=~/logs/ansible-$(date +%Y%m%d_%H%M%S).log
